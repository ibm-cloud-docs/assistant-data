...
- To gather the information that is required to perform a task, try using a node with slots instead of a bunch of separate nodes to elicit information from users. See [Gathering information with slots](/docs/services/assistant-data?topic=assistant-data-dialog-slots).
- For a complex process flow, tell users about any information they will need to provide at the start of the process.
- Understand how your assistant travels through the dialog tree and the impact that folders, branches, jump-tos, and digressions have on the route. See [Dialog flow](/docs/services/assistant-data?topic=assistant-data-dialog-overview#dialog-overview-flow).
...
- To jump to a node in the same branch as the current node, use *Skip user input* instead of a *Jump-to*.
  ...
  This choice prevents you from having to edit the current node's settings when you remove or reorder the child nodes being jumped to. See [Defining what to do next](/docs/services/assistant-data?topic=assistant-data-dialog-overview#dialog-overview-jump-to).
- Before you enable digressions away from a node, test the most common user scenarios. And be sure that likely digressed-to nodes are configured to return. See [Digressions](/docs/services/assistant-data?topic=assistant-data-dialog-runtime#dialog-runtime-digressions).
...
- Back up your skill frequently. See [Downloading a skill](/docs/services/assistant-data?topic=assistant-data-skill-add#skill-add-download).
