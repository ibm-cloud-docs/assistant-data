...
- **NLU**: A Natural Language Understanding interface that the Store microservice uses to initiate machine learning training. This service is an entry point to the language understanding pipeline, which is where text is analyzed to find intent and entity mentions. The Store microservice calls the NLU microservice by using the Litelinks protocol. The NLU microservice controls the lifecycle of the machine learning models. It maps the workspace ID that is used by the Store microservice to the machine language understanding pipeline's internal ID. The NLU microservice uploads the training data for machine learning models to the MinIO object storage. When a language model needs to be trained, because intents were edited for example, the NLU microservice calls the Master microservice. When the updated model is available, NLU calls the TAS microservice to analyze the user input that is provided by the customer. The workspace ID has a UUID format. The internal IDs of models in the language understanding pipeline typically use the syntax, `vn-[0-9a-f]*-[0-9a-f]*- [0-9a-f]*`.

- **Recommends**: Supports recommendations from Watson, such as dictionary-based entity synonyms and intent conflicts. Used only at authoring time from the web UI. When a user requests synonym recommendations for an entity, for example, the UI microservice calls the Store microservice. The Store microservice forwards the request to the Recommends microservice. The Recommends microservice looks up the synonyms for the current word by using the embeddings that are stored in MongoDB, and stores them in a Redis cache. A similar workflow is used to identify intent conflicts in a dialog skill.

- **SIREG**: Tokenizes user input that is sumbmitted to the system. SIRE stands for Statistical Information and Relation Extraction. SIREG runs SIRE tokenizers as microservices with a gRPC interface. For most of the supported Romance languages, the language understanding pipeline has a built-in library for tokenization. Generally, and to oversimplify it, each word in the user input is treated as a separate token. However, for supported languages with a more complex syntax, namely Korean, Chinese, Japanese, and German, SIREG is needed. SIREG provides extra resources that are needed to tokenize the input properly. For each complex language that is configured to be added at installation time, one or more separate SIREG pods are added. The pods are started only if a complex language is installed. SIREG microservice is called from the training pods and from the TAS microservice by using gRPC.

- **Skill-conversation**: Handles v2 API requests that are submitted to the system. If the user input request uses the v2 message API, then the Store microservice retrieves session state infomation from Redis, and then calls the appropriate skill microservice, either this microservice or the Skill-search microservice. When the Store microservice calls this microservice, it does so over HTTPS REST. This mircoservice essentially converts the data from the format used in the v2 message API to the format used by the V1 API (a format that is understood by the underlying workspace object), and then calls the Store microservice back. This microservice performs a similar conversion, except from v1 to v2, on the message response.

- **Skill-search**: Manages API calls to a {{site.data.keyword.discoveryshort}} instance in the same cluster. If the user input request uses the v2 message API, then the Store microservice retrieves session state infomation from Redis, and then calls the appropriate skill microservice, either this microservice or the Skill-conversation microservice. When the assistant that is being processed has a search skill, the Store microservice calls this microservice over HTTPS REST. When a search query is triggered, this microservice calls a {{site.data.keyword.discoveryshort}} instance in the same cluster and converts the output returned by {{site.data.keyword.discoveryshort}} to the v2 API message schema.

- **Store**: Handles all assistant API calls. This microservice either processes the request itself or calls the other {{site.data.keyword.conversationshort}} microservices that are needed to process the request. For example, if a customer submits user input, a message API call is sent to the store. The store first calls the NLU microservice to analyze the user input and identify any intent and entity references in the input. The Store microservice then calls the Dialog microservice to generate the appropriate response to return to the customer. The Store microservice stores the assistants, skills, and workspace definitions in a PostgreSQL database. The Store microservice saves the state of the session (from the v2 API) in the Redis data store.
...
The Postgres data store is based on stolon, which is a cloud-native PostgreSQL manager for PostgreSQL high availability. (For more information, see the [GitHub sorintlab repo](https://github.com/sorintlab/stolon).) The Postgres store consists of three kinds of pods:
...
During the installation process, the Postgres database name is defined, as is the username and password of its administrator. You can change the details for the store database if you want, by overriding configuration settings in the `value.yaml` configuration file.
...
|------------------|----------------------------|---------------|
| Store administrator username | global.postgres.store.auth.user | `store_icp_${release-name}` |
| Secret with the password that corresponds to the administrative user | global.postgres.store.auth.authSecretName | null |
